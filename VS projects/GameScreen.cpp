 ...
 Vector2i res)
{
 ...
 auto guip = unique_ptr<GameUIPanel>(new GameUIPanel(res));
 addPanel(move(guip), smrc, m_GIH);
 auto m_GOIH = make_shared<GameOverInputHandler>();
 auto gouip = unique_ptr<GameOverUIPanel>(new GameOverUIPanel(res));
 ...
 m_BackgroundTexture.loadFromFile("world/background.png");
